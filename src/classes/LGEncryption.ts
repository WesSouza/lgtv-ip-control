...
export class LGEncryption {
  ...
  decrypt(cipher: Buffer) {
    const ecbDecypher = createDecipheriv('aes-128-ecb', this.derivedKey, Buffer.alloc(0));
    ecbDecypher.setAutoPadding(false);
    const iv = ecbDecypher.update(cipher.slice(0, this.settings.encryptionKeyLength));

    const cbcDecypher = createDecipheriv('aes-128-cbc', this.derivedKey, iv);
    cbcDecypher.setAutoPadding(false);
    return cbcDecypher.update(cipher.slice(this.settings.encryptionKeyLength)).toString();
  }
}
